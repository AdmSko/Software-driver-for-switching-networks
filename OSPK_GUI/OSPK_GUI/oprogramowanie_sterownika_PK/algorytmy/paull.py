import copy
from oprogramowanie_sterownika_PK.narzedzia.program_tools import znajdz_klucz_w_slowniku

def algorytm_paulla(struktura, wl, wp, x, y, slownik_polaczen, nr_komutatora_we,
                    nr_komutatora_wy, nr_lacza_we, nr_lacza_wy):
    """
        Odpowiedzialna za algorytm przestrojeń Paull'a dla pól 3-sekcyjnych.
    ...
    """
    # KROK 1: tworzymy i aktualizujemy macierz na podstawie słownika z połączeniami, macierz pomocnicza
    macierz_paulla, macierz_pomocnicza = stworz_macierz_paulla(wl[1], slownik_polaczen)
    ...
    macierz_paulla, macierz_pomocnicza_komutatory, lista_zmian = przestrajanie_macierzy(
    ...
    wiadomosc += f"\nd{id_klucz} = {{{pozycje_str}}}"

    return slownik_polaczen, struktura, wiadomosc


def stworz_macierz_paulla(r, slownik_polaczen):
    """
    ...
    :return: Stworzona macierz Paull'a i wartości symboli A i B
    """
...
def zamiana_wartosci_na_symbole(macierz_paulla, nr_komutatora_we, nr_komutatora_wy, m):
    ...
    """
    # Krok 1: Zbieramy wszystkie wartości, które występują w wierszu switch_nbr_in
    uzyte_w_wierszu = set()
    for j in range(len(macierz_paulla[nr_komutatora_we])):
        if macierz_paulla[nr_komutatora_we][j] is not None:
            # Rozdzielamy ciągi, takie jak '0,1' na pojedyncze elementy
            for wartosc in macierz_paulla[nr_komutatora_we][j].split(','):
                uzyte_w_wierszu.add(wartosc.strip())

    # Krok 2: Wybieramy wartosc_B, czyli wartość, która nie występuje w wierszu
    wartosc_B = None
    ...
    # Krok 3: Zamieniamy w całej macierzy wartości wartosc_B na "B"
    for i in range(len(macierz_paulla)):
        for j in range(len(macierz_paulla[i])):
            if macierz_paulla[i][j] is not None and wartosc_B in str(macierz_paulla[i][j]):
                # Zamieniamy tylko, jeśli wartosc_B (np. '0', '1', ...) jest zawarte w komórce
                macierz_paulla[i][j] = 'B'

    # Krok 4: Zbieramy wszystkie wartości, które występują w kolumnie switch_nbr_out
    ...
    # Krok 5: Wybieramy wartosc_A, czyli wartość, która nie występuje w kolumnie
    wartosc_A = None
    for wartosc in range(m):  # Zakładając, że wartości w kolumnie są w zakresie 0-m
        wartosc_A_str = str(wartosc)  # Zamieniamy na string
        if wartosc_A_str not in uzyte_w_kolumnie and wartosc_A_str != wartosc_B:
            wartosc_A = wartosc_A_str
            break

    # Krok 6: Zamieniamy w całej macierzy wartości wartosc_A na "A"
    for i in range(len(macierz_paulla)):
        for j in range(len(macierz_paulla[i])):
            if macierz_paulla[i][j] is not None and wartosc_A in str(macierz_paulla[i][j]):
                # Zamieniamy tylko, jeśli wartosc_A (np. '0', '1') jest zawarte w komórce
                macierz_paulla[i][j] = 'A'

    return macierz_paulla, wartosc_A, wartosc_B


def przestrajanie_macierzy(macierz_paulla, nr_komutatora_we, nr_komutatora_wy, macierz_pomocnicza_komutatory, wartosc_A, wartosc_B):
    """
    ...
    :param wartosc_A: Wartość, która należy do symbolu A
    :param wartosc_B: Wartość, która należy do symbolu B
    ...
    # rozpoczęcie od dodanego połączenia matrix[switch_nbr_in, switch_nbr_out]
